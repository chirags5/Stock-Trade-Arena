...
export default function SignalFeed({ API, prices, onTrade }) {
        ...
        stockList.forEach(stock => {
          if (!stock.price) {
            axios.get(`${API}/price/${stock.ticker}`).then(r => {
              if (r.data?.price) setStocks(prev => prev.map(s => s.ticker === stock.ticker ? { ...s, price: r.data.price } : s));
            }).catch(() => {});
          }
        });
      }
    } catch (e) { console.error('Stocks fetch error:', e); } finally { setLoading(false); }
  }

  function getQty(ticker) { return quantities[ticker] || 1; }
  function showToast(msg, isError = false) { setToast({ msg, isError }); setTimeout(() => setToast(null), 3500); }
  ...
  async function handleTrade(stock, direction) {
    ...
    if (!price) {
      ...
      } catch (e) { return showToast(`Could not fetch price for ${stock.ticker}`, true); }
    }
    if (!price) return showToast(`No live price for ${stock.ticker}. Market may be closed.`, true);

    setActing(prev => ({ ...prev, [key]: true }));
    try {
      await axios.post(`${API}/trade`, { ticker: stock.ticker, direction, qty, buy_price: price });
  ...
  async function fetchPriceOnHover(stock) {
    ...
    } catch (e) {} finally { hoverFetchRef.current[ticker] = false; }
  }
            ...
            <div style={s.tableBody}>
              {stocks.length === 0 ? (
                ...
                stocks.map(stock => {
                  ...
                  return (
                    <div key={stock.ticker} style={s.row} onMouseEnter={() => fetchPriceOnHover(stock)}>
                      <div style={s.cTicker}>
                      ...
                      <div style={s.cPrice}>
                        {livePrice ? <span style={s.priceText}>₹{livePrice.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span> : <span style={s.noPrice}>—</span>}
                      </div>
                      
                      <div style={s.cQty}>
                        <input type="number" min="1" value={qty} onChange={e => setQty(prev => ({ ...prev, [stock.ticker]: Math.max(1, parseInt(e.target.value) || 1) }))} style={s.qtyInput} />
                      </div>
                      
                      <div style={s.cActions}>
                        {cost && <span style={s.costText}>₹{cost.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>}
                        <button onClick={() => handleTrade(stock, 'BUY')} disabled={acting[buyKey]} style={{ ...s.actionBtn, ...s.buyBtn, opacity: acting[buyKey] ? 0.6 : 1 }}>
                          {acting[buyKey] ? '...' : 'Buy'}
                        </button>
                        <button onClick={() => handleTrade(stock, 'SHORT')} disabled={acting[shortKey]} style={{ ...s.actionBtn, ...s.shortBtn, opacity: acting[shortKey] ? 0.6 : 1 }}>
...
const s = {
  ...
  suggestionBox: { 
    ...
    zIndex: 100, marginTop: '8px', padding: '6px' 
  },
  ...
  cActions: { width: '220px', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'flex-end' },
  ...
  actionBtn: {
    ...
    transition: 'all 0.2s', 
    cursor: 'pointer',
    color: '#ffffff' // Keeping buttons white text in both modes for contrast against colored backgrounds
  },
  buyBtn: { backgroundColor: '#4caf50' }, 
  shortBtn: { backgroundColor: '#ef5350' }, 
